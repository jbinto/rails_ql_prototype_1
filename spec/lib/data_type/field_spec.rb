...
describe RailsQL::DataType::Field do
  ...
  describe "#resolved_models" do
    context "when parent_data_type is present" do
      before :each do
    ...
    context "when parent_data_type#model is not present" do
      before :each do
        allow(field).to receive(:parent_data_type).and_return nil
      end

      context "when nullable is false" do
        it "raises an error" do
          expect(field).to receive(:nullable?).and_return false
          expect{field.resolved_models}.to raise_error
        end
      end
