    ...
    class Visitor < GraphQL::Parser::Visitor
        ...
        if within_inline_fragment?
          if within_fragment_definition?
            ...
            @union_type_builder_stack.push(
              current_type_builder.add_child_builder! name: node.value
            )
          end
        end
      end
      ...
      def visit_field_name(node)
        if within_inline_fragment?
          ...
          child_type = current_type_builder.add_child_builder! name: node.value
        ...
        if within_fragment_definition?
          ...
          @union_type_builder_stack.push(
            (@union_type_builder_stack.last || current_type_builder)
              .add_child_builder!(name: node.value)
          )
        end
      end
      ...
      def end_visit_object_value(node)
        if @input_object_key_stack.size == 1
          ...
          current_type_builder.add_arg_builder!(
            name: @input_object_key_stack.first.to_sym,
            model: @input_object[@input_object_key_stack.first]
          )
          @input_object = nil
        end
      ...
      def visit_fragment_spread_name(node)
        if within_fragment_definition?
          ...
          current_type_builder.add_fragment! name: node.value
        elsif within_type_within_fragment_definition?
      ...
      def resolve_fragments!
        ...
        @type_builders.each do |type_builder|
          @fragments.each do |fragment|
            if type_builder.fragments.include?(fragment[:name])
      ...
      def apply_fragment_to_type_builder(fragment, type_builder)
        ...
        fragment[:args].each do |name, value|
          type_builder.add_arg_builder! name: name, model: value
        end
