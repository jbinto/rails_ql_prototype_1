class UserDataType < RailsQL::DataType::Base
  ...
  has_many(:to_dos,
    args: [:status],
    query: ->(args, child_query) {query.eager_load(:to_dos).merge child_query.where(args)},#.merge child_query.where(args)},
    resolve: ->(args, child_query) {model.to_dos.to_a.first}
  )
